...
### Проблема 6: Ошибки кодировки в терминале (кракозябры вместо русского)

**Симптомы:**
- В консоли вместо русского текста — вопросительные знаки, ромбики или `UnicodeEncodeError`.
- Часто при запуске agent9, загрузке `config/.env` с русскими комментариями или при выводе логов.

**Решение:**

1. **Запуск через батник** (рекомендуется): используйте `run_agents.bat` или `scripts\run_image_api.bat` — в них уже стоит `chcp 65001` (UTF-8 для консоли).

2. **Запуск Python напрямую из PowerShell:** скрипты agent9 и `image_generate_api.py` при старте переключают вывод в UTF-8 (`sys.stdout.reconfigure(encoding="utf-8")`). Если ошибка остаётся, перед запуском выполните:
   ```powershell
...
3. **Файл `config/.env`:** сохраняйте в кодировке **UTF-8 без BOM** (в Notepad: «Сохранить как» → кодировка «UTF-8»). Иначе загрузка переменных с русскими комментариями может приводить к сбоям.

4. **Терминал Cursor/VS Code:** в настройках терминала можно задать кодировку UTF-8 или использовать встроенный терминал (обычно уже UTF-8).
