...
# Запуск: powershell -ExecutionPolicy Bypass -File "D:\content-factory\git-entuziastov.ps1" [-project factory|vps]
...
# 2.x Автотеги по файлам (feat: [#agents] сообщение)
...
if ($isFactory) {
    ...
    elseif ($changedFiles -match "^(materials/|prompts/|output/)") { $tag = "[#контент]" }
} elseif ($isVps) {
    $tag = "[#сайт]"
}
if ($tag -ne "") {
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Ошибка git add. Проверь права доступа или .gitignore."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    if ([string]::IsNullOrWhiteSpace($errStatus)) {
        Write-Host "Коммит не создан: похоже, после git add нечего коммитить (nothing to commit)."
    } else {
        Write-Host "Ошибка git commit. Проверь hooks/конфликты."
    }
    exit 1
}
