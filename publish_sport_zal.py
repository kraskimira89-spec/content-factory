...
# Загрузка переменных из config/.env (WP_URL, WP_USERNAME, WP_APP_PASSWORD)
try:
...
# ─────────────────────────────────────────────
# 1. НАСТРОЙКИ — из config/.env или переменные окружения
# ─────────────────────────────────────────────
...
def _get_wp_credentials():
    ...
    # Basic Auth допускает только Latin-1
    try:
...
# По аналогии с конференц-залом: классы landing__*, чередование секций, карточки coral/teal
# ─────────────────────────────────────────────
...
""")
    ...
    # ── 2. ДЛЯ КОГО (audience cards, как конференц-зал) ──
...
""")
    ...
    # ── 3. ПРЕИМУЩЕСТВА (advantages, как конференц-зал) ──
    ben_html = "".join(
...
""")
    ...
    # ── 6. ТАРИФЫ (landing__tariffs как конференц-зал) ──
    price_html = ""
...
""")
    ...
    # ── 7. СИНЕРГИЯ (feature cards coral/teal) ──
...
""")
    ...
    # ── 8. КАК НАЧАТЬ (booking steps, как конференц-зал) ──
    steps_html = "".join(
